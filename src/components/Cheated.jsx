...
function Cheated() {
                   ...
                   if(result.status=='OK'){
                    try {
                        const response = await fetch('http://localhost:3000/user', {
                            ...
                            body: JSON.stringify({ username: UserId.toLowerCase()  })
                        });
                        console.log('success!!')
                    }catch(err){
    ...
    if (error) {
        return <p className='flex flex-col items-center justify-center text-lg font-sans text-white m-5'>{error}</p>;
    }

    if (queNum === 0 && UserId !== '') {
        return <p className=' flex flex-col items-center justify-center text-lg font-sans text-white m-5'>{UserId} is a legit person!!</p>;
    }
